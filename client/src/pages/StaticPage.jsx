...
const pageContent = {
  faq: {
    ...
    items: [
      ...
      ['Do services include warranty?', 'Completed workmanship includes a 3 month warranty. Parts follow supplier warranty.'],
  ...
  terms: {
    ...
    items: [
      ...
      ['Warranty', 'Warranty covers workmanship for 3 months and excludes misuse, external damage, and unrelated faults.'],
    ],
  },
  success: {
      ...
      <main className="bg-gray-50 py-10 dark:bg-gray-950">
        <section className="mx-auto max-w-5xl px-4 sm:px-6">
          <div className="rounded-2xl bg-navy-900 p-8 text-white shadow-xl">
            ...
            <h1 className="mt-3 text-4xl font-extrabold">{page.heading}</h1>
            <p className="mt-3 max-w-2xl text-sm leading-7 text-white/75">{page.description}</p>
          </div>
          <div className="mt-8 grid gap-4">
            {page.items.map(([title, text]) => (
              <article key={title} className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-gray-900">
                <h2 className="flex items-center gap-2 font-bold text-gray-950 dark:text-white">
                  {type === 'success' ? <CheckCircle2 className="text-emerald-500" size={19} /> : <ShieldCheck className="text-amber-600" size={19} />}
                  {title}
                </h2>
                <p className="mt-2 text-sm leading-7 text-gray-600 dark:text-gray-300">{text}</p>
              </article>
            ))}
          </div>
